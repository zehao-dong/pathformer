...
def arg_parse():
    ...
    parser.add_argument('--batch-size', dest = 'batch_size', type = int, default=8,
                help = 'Batch size.')
    parser.add_argument('--epochs', dest = 'num_epochs', type = int, default=80,
...
if __name__ == '__main__':
    ...
    _, mayo_y, _ = ad_preprocess(mayo,args.n_nodes)
    rosmap = pd.read_csv('./data/RosMap_Exp_1a.csv',sep=' ')
    _, rosmap_y, _ = ad_preprocess(rosmap,args.n_nodes)
    mayo = pd.read_csv('./data/Mayo_Exp_1a.csv',sep=' ')
    rosmap = pd.read_csv('./data/RosMap_Exp_1a.csv',sep=' ')
    gene_list = [
    'FEZ1', 'SLC48A1', 'MYL6B', 'AP2S1', 'ARPC5L', 'B4GAT1', 'HINT1', 'MAP1LC3B', 'KCNJ4', 'BANF1', 'DBNDD2', 'SDHC', 'MRPS24', 'GOT1', 'HEPACAM', 
    'SRP14', 'HID1', 'CTNNA1', 'PTK2B', 'SEPTIN3', 'SREBF1', 'RNF141', 'JUND', 'SLC1A3', 'SMDT1', 'GNAI1', 'KCNQ2', 'DLGAP1', 'SLC4A10', 'EIF4G2', 
    'SCHIP1', 'TUBB4B', 'PICALM', 'CLSTN1', 'MYO15B', 'LDB2', 'CFL2', 'SEM1', 'NTRK2', 'H3-3A', 'ARPC1A', 'CNTNAP2', 'ATP2B1', 'PGRMC1', 'NRCAM', 
    'TRIM2', 'ANK2', 'CA2', 'VAMP3', 'ATP6V1D', 'SQSTM1', 'KLK6', 'PSMC1', 'SLC22A17', 'AIF1L', 'FABP7', 'YBX1', 'ATCAY', 'WLS', 'DDIT4', 'TCEAL2', 
    'ARHGDIG', 'SSR4', 'PRDX5', 'MICAL2', 'NDUFA3', 'ATP13A2', 'NELL2', 'CNP', 'MT-ND2', 'SNURF', 'CERS1', 'ID3', 'PCP4', 'ELOB', 'CNIH1', 'TESPA1', 
    'GSTM5', 'SEPTIN8', 'EDF1', 'CDC42', 'DYNLT3', 'PLEKHG3', 'XRCC6', 'CADM3', 'DOC2A', 'CTNNA2', 'APOE', 'HMGN2', 'OTUB1', 'FAM131B', 'FAXDC2', 
    'PSEN1', 'ASPHD1', 'MT1X', 'WBP2', 'GPM6A', 'CNRIP1', 'TMEM205', 'NME1', 'SRCIN1', 'FAM49B', 'HSD11B1L']
    Ngenes = len(gene_list)
    raw_data1 = mayo.loc[gene_list]
    raw_data2 = rosmap.loc[gene_list]
    raw_data = pd.concat([raw_data1, raw_data2],axis=1)
    raw_y = np.array(list(mayo_y) + list(rosmap_y))
    ...
    X = torch.FloatTensor(raw_data.values).permute(1,0)#将tensor的维度换位 ex  3*2 to 2*3
    ...
    res_dir = './visual' + '/results_{}'.format(fold_idx)
    ...
    for i in range(1,EPOCH+1):
        ...
        for k in pbar:
            ...
            loss = loss_func(pred, batch_y) # Computes the loss between the predicted values (pred) and the actual target values (batch_y)
            ...
            pbar.set_description('Epoch: {0} total_loss:{1:.3f} acc: {2:.4f}'.format(i, loss.item(), acc))
