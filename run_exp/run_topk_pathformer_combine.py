...
if __name__ == '__main__':
    ...
    X_1 = torch.FloatTensor(raw_data_mayo.values).permute(1,0)[:-1]#将tensor的维度换位 ex  3*2 to 2*3
    Y_1 = torch.LongTensor(raw_y_mayo)
    X_2 = torch.FloatTensor(raw_data_rosmap.values).permute(1,0)[:-1]#将tensor的维度换位 ex  3*2 to 2*3
    Y_2 = torch.LongTensor(raw_y_rosmap)

    X = torch.cat([X_1,X_2],dim=0)#将tensor的维度换位 ex  3*2 to 2*3
    Y = torch.cat([Y_1, Y_2])
    ...
    for fold_idx in range(args.fold-1):
        ...
        res_dir = './combine_mayo_rosmap' + '/results_{}'.format(fold_idx)
