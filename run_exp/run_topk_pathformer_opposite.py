...
if __name__ == '__main__':
    ...
    print('start processing mayo')
    mayo = pd.read_csv('./data/Mayo_Exp_1a.csv',sep=' ')
    raw_data_mayo, raw_y_mayo, _ = ad_preprocess(mayo,args.n_nodes)
    new_candi_genes = []
    for gene in tgt_ad_genes:
        if gene not in raw_data_mayo.index:
            print(gene)
            if gene in mayo.index:
                new_candi_genes.append(gene)
    pad_df = mayo.loc[new_candi_genes]
    raw_data_mayo = pd.concat([raw_data_mayo, pad_df],axis=0)
    Ngenes = raw_data_mayo.shape[0]
    gene_list = list(raw_data_mayo.index) #show top 3000 gene names based on variance

    print('start processing rosmap')
    ...
    raw_y_rosmap = np.array([1 if x.startswith('control') else 0 for x in columns])#y贴标签1or0
    rosmap['var'] = var
    raw_data_rosmap = rosmap.loc[data_mayo.index]
    #print(raw_data)
    #raw_data, raw_y, _ = ad_preprocess(rosmap,args.n_nodes)
    new_candi_genes = []
    for gene in tgt_ad_genes:
        if gene not in raw_data_rosmap.index:
            print(gene)
            if gene in rosmap.index:
                new_candi_genes.append(gene)
    pad_df = rosmap.loc[new_candi_genes]
    raw_data_rosmap = pd.concat([raw_data_rosmap, pad_df],axis=0)
    ...
    adj_mayo = adj_generate(raw_data_mayo, biogrid_intact, Ngenes, reverse = False)#make adj matrix 3000*3000


    # X_train, Y_train, X_test, Y_test = dataset_prepare(data_mayo, y_mayo)
    if args.dataset == "mayo":
        X_train = torch.FloatTensor(raw_data_mayo.values).permute(1,0)[:-1]#将tensor的维度换位 ex  3*2 to 2*3
        Y_train = torch.LongTensor(raw_y_mayo)
        N = X_train.size(0)-1 #158 remove the var column
        idx = list(range(N))       
        random.shuffle(idx)      
        X_train = X_train[torch.LongTensor(idx)]#根据idx乱序排列X里的rows
        Y_train = Y_train[torch.LongTensor(idx)]#同理

        X_test = torch.FloatTensor(raw_data_rosmap.values).permute(1,0)[:-1]#将tensor的维度换位 ex  3*2 to 2*3
        Y_test = torch.LongTensor(raw_y_rosmap)
        
    elif args.dataset == "rosmap":
        X_train = torch.FloatTensor(raw_data_rosmap.values).permute(1,0)[:-1]#将tensor的维度换位 ex  3*2 to 2*3
        Y_train = torch.LongTensor(raw_y_rosmap)
        N = X_train.size(0)-1 #158 remove the var column
        idx = list(range(N))       
        random.shuffle(idx)      
        X_train = X_train[torch.LongTensor(idx)]#根据idx乱序排列X里的rows
        ...
        X_test = torch.FloatTensor(raw_data_mayo.values).permute(1,0)[:-1]#将tensor的维度换位 ex  3*2 to 2*3
        Y_test = torch.LongTensor(raw_y_mayo)

    
    if args.reg:
        res_dir = './' + args.dataset + '_opposite/results_reg'
    else:
        res_dir = './' + args.dataset + '_opposite/results'

    if not os.path.exists(res_dir):
        os.makedirs(res_dir) 
    
    if args.K == 0:
        pathformer = Pathormer(n_copy=args.n_copy, copy_dim=args.copy_dim, d_model=args.d_model, n_head=args.n_head, d_ff=args.d_ff, drop_out=args.dropout, n_layers=args.n_layers, hidden_dim=args.hidden_dim,
                        out_hidden=args.out_hidden, Nnodes=Ngenes, out_dim=args.out_dim, device=device)
    else:
